...
export function deleteFromLocalStorage(target) {
    ...
    try {
        if (target.closest(".card__btn")) {
           ...
           const [elementImgWrapperm, elementDescription] = [...cardDivParent.children];
           ...
           [...elementDescription.children].map(item2 => {
               if (item2.classList.contains("card__title")) {
                ...
                const newLocalStorageData = arrLocalStorage.filter(itemFilter => itemFilter.title.toLowerCase().trim() != str) 
